...
public struct WorleyNoiseGenerator
{
    ...
    int TerrainTypeIndex (float cellNoise)  // used to compare current cell's topography height with adjacent cell's height in worley surface cell generation, if they are same no need for edge data
    {
        if (cellNoise >= 0.9f)
            return 0;
        else if (cellNoise >= 0.7f)
            return 1;
        else if (cellNoise >= 0.4f)
            return 2;
        else if (cellNoise >= 0.2f)
            return 3;
        else
            return 4;
    }
